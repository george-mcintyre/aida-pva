...
--
--  Name:  aida_caService_namesUpdate_qa.sql                        
--
...
insert into aida_names( state, instance, attribute)
	select 'N', &inst_map, &attr_map from 
           pvudb.pvu_rec@mccqatoslacprod_pvudb.slac.stanford.edu
	...
	set state = 'S' where
		...
		id in ( select id from aida_names, aida_directory 
			where aida_names.id = aida_directory.name_id and
			      aida_directory.service_id in 
				&ca_contributor_set) and
		exists ( select * from pvudb.pvu_rec@mccqatoslacprod_pvudb.slac.stanford.edu s 
