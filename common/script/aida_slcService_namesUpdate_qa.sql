...
--
--  Name:  aida_slcService_namesUpdate_qa.sql                        
--
...
--         02-Mar-2010, Greg White (greg)
--             Removed 2nd half of the conjunction in the exists clause in block 3). 
--             The conjunction was redundant, since both sides were identical. 
...
values('MCCQA', 'AIDA', 'SLC DB PMU names', 'START', NULL, 'Update',0);
...
-- Replace truncate and drops with delete, to avoid DDL and hence inter-process 
-- interference.
delete from aida_secndx;

-- Note: On MCCQA secndx is a synonym to a dblink to irmisdb.secndx@SLACPROD. This is
-- the only line different in this file to the same named file withot the "_qa".
insert into aida_secndx
	select &inst_map, &attr_map from secndx;
...
insert into controls_global.data_validation_tab(
INST, SCHEMA_NM, PROCESS_NM, STAGE, STATUS_CHK, MSG, PROCESS_GRP_NO)
values('MCCQA', 'AIDA', 'SLC DB PMU names', 'FINISH', 'SUCCESS', 'Update',0);
