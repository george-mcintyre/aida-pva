...
--==============================================================
--
--  Abs:   Populate the Aida database tables for the Aida magnet
--         server. 
--
--  Name:  set_aida_slc_magnet.sql
--
--  Rem:   This script populates Aida database tables for the Aida
--         magnet server from instance/attribute pairs read from a 
--         text file.  The text file name is "aida_magnet_load.dat"
...
--         the Perl script create_aida_magnet_load_file.pl using the
--         input files "dgrpnames.dmp" and "magnetprim.dmp".  These 
--         files are generated in the VMS NFS_NDB directory by the VMS
--         command file slccom:slcdb2oracle.submit, which is run at
...
--         deleted from these tables if the Aida magnet instance/attribute
--         pairs that were formerly in the "aida_magnet_load.dat"
--         file are no longer in the new version of this file.  The Oracle
--         tables aida_old_slc_magnet and aida_new_slc_magnet are used by this
...
declare
    ...
    type slc_magnet_ids_type is ref cursor;
    slc_magnet_ids      slc_magnet_ids_type;

    type old_slc_magnet_ids_type is ref cursor;
    old_slc_magnet_ids  old_slc_magnet_ids_type;

    type new_slc_magnet_ids_type is ref cursor;
    new_slc_magnet_ids  new_slc_magnet_ids_type;

/* The PL/SQL routine delete_slc_magnet_id is used by the main routine
   to delete the row in the Oracle aida_directory table associated
   with the specified id number and SLC magnet server service number.
...
-------------------------------------------------------------------- */
procedure delete_slc_magnet_id (slc_magnet_id in integer,
...
begin
    ...
    /***DEBUG
    DBMS_OUTPUT.PUT_LINE('entering delete_slc_magnet_id with id = ' ||
        to_char(slc_magnet_id));
    */

    execute immediate 'delete from aida_directory where name_id = ' ||
        to_char(slc_magnet_id) || ' and service_id = ' || to_char(service_num);

    execute immediate 'select count(*) from aida_directory where name_id = ' ||
        to_char(slc_magnet_id) into adir_entry_found;
...
begin
    ...
    dbms_output.put_line('Log for populating database for Aida SLC Magnet server.  User name: ' || user_name);
    ...
    /* Truncate any existing rows from the Oracle tables aida_old_slc_magnet
       and aida_new_slc_magnet used as storage space in this script.
    ---------------------------------------------------------------------- */
    execute immediate 'truncate table aida_old_slc_magnet';

    execute immediate 'truncate table aida_new_slc_magnet';

    /* Retrieve the service id number from the aida_services table
       associated with the "SLCMagnet" data provider service.
    -------------------------------------------------------------- */
    service_name := 'SLCMagnet';
    ...
    */
    ...
    /* Populate the aida_old_slc_magnet table with all of the name_id
       numbers in rows of the aida_directory associated with the
       "SLCMagnet" data provider service.
    --------------------------------------------------------------- */
    sql_stmt_1 := 'select name_id from aida_directory where service_id =  ' ||
        to_char(service_num);

    status := 1;
    open slc_magnet_ids for sql_stmt_1;
    while (status = 1) loop
        fetch slc_magnet_ids into name_id_num;
        exit when slc_magnet_ids%NOTFOUND;

        /***DEBUG
        DBMS_OUTPUT.PUT_LINE('existing name_id_num = ' || to_char(name_id_num));
        */

        execute immediate 'insert into aida_old_slc_magnet values (' ||
            to_char(name_id_num) || ')';

    end loop;

    /* Open the file "aida_magnet_load.dat" for reading.  This file
       ...
       contains the Aida magnet server instance/attribute pairs.  There
    ...
    while (status = 1) loop
        ...
        */
        ...
        /* Retrieve the id for the newly inserted or previously existing row in
           ...
           names.  If this id number is not already in the aida_new_slc_magnet table
           ...
           aida_new_slc_magnet table, which will contain the aida_names id number
        ...
        */
        ...
        execute immediate 'select count(*) from aida_new_slc_magnet where name_id = ' ||
            to_char(name_id_num) into row_found;

        if (row_found = 0) then

            execute immediate 'insert into aida_new_slc_magnet values (' ||
                to_char(name_id_num) || ')';

        end if;

        /* Determine if there are any rows in the aida_directory table containing
           the aida_names id number and SLCMagnet service id number.
    ...
    /* The Aida database has now been populated with information associated
       ...
       table) will be deleted if there are Aida magnet server entries in the
       ...
       deleted is to compare the sorted contents of the aida_old_slc_magnet
       and aida_new_slc_magnet processing tables.  Any aida_names id number
       in the aida_old_slc_magnet table that is not in the aida_new_slc_magnet
       table is an id associated with a Aida magnet server entry that is in the
       ...
       calling the routine delete_slc_magnet_id (the associated aida_names
       row may be deleted also).
    ------------------------------------------------------------------------ */
       
    sql_stmt_2 := 'select name_id from aida_old_slc_magnet order by name_id';

    open old_slc_magnet_ids for sql_stmt_2;

    sql_stmt_3 := 'select name_id from aida_new_slc_magnet order by name_id';

    open new_slc_magnet_ids for sql_stmt_3;
    ...
    /* The loop will be exited when there are either no more entries in the
       aida_old_slc_magnet table that have not been processed or there are no
       more entries in the aida_new_slc_magnet table that have not been
       processed.
    ----------------------------------------------------------------------- */
    while (status = 1) loop

        /* If this the first time through the loop, read an aida_names id
           from both the aida_old_slc_magnet and aida_new_slc_magnet sorted
           tables.
        ----------------------------------------------------------------- */
        if (first_read = 1) then
            ...
            fetch old_slc_magnet_ids into old_name_id_num;
            if (old_slc_magnet_ids%NOTFOUND) then
                more_old_rows := 0;
                exit;
            end if;

            fetch new_slc_magnet_ids into new_name_id_num;
            if (new_slc_magnet_ids%NOTFOUND) then
                more_new_rows := 0;
                exit;
            end if;
        ...
        */
        ...
        /* The current aida_names id numbers from the sorted aida_old_slc_magnet
           and aida_new_slc_magnet tables are compared.  If they are equal, read
           an aida_names id from the next entries in the sorted
           aida_old_slc_magnet and aida_new_slc_magnet tables.
        ---------------------------------------------------------------------- */
        ...
        /* If the current aida_names id number from the sorted
           aida_old_slc_magnet table is less than the current aida_names id
           number from the aida_new_slc_magnet table, then there is
           not a match in the aida_new_slc_magnet table for the current
           aida_names id number from the aida_old_slc_magnet table.
           Therefore, this aida_names id number is not associated with
           a instance/attribute pair in the input file just processed
           and the Aida magnet server information in the Aida database for this
           id may be deleted.  After calling delete_slc_magnet_id to do
           this deletion, get the next current aida_old_slc_magnet
           aida_names id number from the sorted table.
        ------------------------------------------------------------------ */
        elsif (old_name_id_num < new_name_id_num) then

            delete_slc_magnet_id(old_name_id_num, service_num);
        ...
        /* If the current aida_names id number from the sorted
           aida_old_slc_magnet table is greater than the current aida_names
           id number from the aida_new_slc_magnet table, get the next
           current aida_new_slc_magnet aida_names id number from the
           sorted table.
        ----------------------------------------------------------------- */
        elsif (old_name_id_num > new_name_id_num) then
    ...
    */
    ...
    /* If there are any remaining entries from the sorted
       aida_old_slc_magnet table that have not been processed,
       loop through them and delete the Aida magnet server 
       information in the Aida database associated with
       each by calling routine delete_slc_magnet_id.
    -------------------------------------------------------- */
    ...
    errcode := 0;
    dbms_output.put_line('set_aida_slc_magnet errcode: ' || to_char(errcode));
